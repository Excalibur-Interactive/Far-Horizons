...
void WM_SetPossibleCommands()
{
	...
	{
	case 0:
		// Rebbebion, неиспользованная иконка доплытия к острову
	...
	if (bDefault)
	{
		if (sti(worldMap.encounter_island)) // Rebbebion, неиспользованная иконка доплытия к острову
		{
			BattleInterface.Commands.EnterToIsland.enable = true;
			Log_SetActiveAction("EnterToIsland");
		}
...
void WM_InitializeCommands()
{
	...
	BattleInterface.Commands.EnterToIsland.selPicNum	= 8;
