// диалог матросов, гуляющих по улицам городов
	...
	switch(Dialog.CurrentNode)
	{
		case "First time":
			if (npchar.quest.meeting == "0")
			{
				...
				link.l1.go = "quests";//(перессылка в файл города)
			...
			else
			{
				...
				link.l1 = RandPhraseSimple("У меня есть вопрос к тебе.", "Мне нужна информация о делах в этой колонии.");
				link.l1.go = "quests";//(перессылка в файл города)
				link.l2 = "Нет, приятель, ничего. Удачи!";
				link.l2.go = "exit";
			}
		...
		case "crew_2":
			...
			{
				link.l1 = "Договорились! Вот здесь вся сумма. Отправляйтесь на мой корабль, он называется '"+pchar.ship.name+"', стоит на рейде. Квартирмейстер выделит вам места в кубрике и даст работу.";
				link.l1.go = "crew_3";
			}
