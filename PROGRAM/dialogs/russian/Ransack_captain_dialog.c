...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
                link.l3 = "Ступайте к квартирмейстеру, скажите пусть выделит рундук на баке. В ближайшей бухте сойдёте на берег.";
			    link.l3.go = "free_withoutFee_2";
			}			
			link.l4 = "Я думаю, из вас получится отличный раб - крепкий и послушный.";
			link.l4.go = "Slave_1";
			link.l5 = "Вы проигнорировали приказ лечь в дрейф и оказали сопротивление, за это будете казнены. квартирмейстер уже получил соответствующие распоряжения.";
		...
		case "second time":
			if(CheckAttribute(NPChar,"Hold_GenQuest") && !CheckAttribute(pchar,"GenQuest.Hold_GenQuest"))
			{
				...
				{				
					...
					case 1:
						...
						if (attrLoc != "none") 
						{
							link.l2 = "Вы проигнорировали приказ лечь в дрейф и оказали сопротивление, за это будете казнены. квартирмейстер уже получил соответствующие распоряжения.";
			...
			else
			{
				...
				else
				{
					...
					{
						link.l4 = "Ступайте к квартирмейстеру, скажите пусть выделит рундук на баке. В ближайшей бухте сойдёте на берег.";
						link.l4.go = "free_withoutFee_4";			
					}	
					else
					{
						link.l4 = "Черт с вами! Ступайте к квартирмейстеру, он получит соответствующие распоряжения, вас высадят в ближайшей бухте. И помните - следующая наша встреча будет последней.";
						link.l4.go = "free_withoutFee_4";
					}
				}				
				link.l5 = "Вы проигнорировали приказ лечь в дрейф и оказали сопротивление, за это будете казнены. квартирмейстер уже получил соответствующие распоряжения.";
		...
		case "free_to_officer_trader":
			...
			else
			{
				...
				link.l1 = "Вот и договорились! Идите к квартирмейстеру - пусть поставит вас на довольствие. После определю вам должность.";	
				link.l1.go = "free_to_officer_Hire";
		...
		case "Slave_2":
            ...
            if (rand(5) == 1)
            {
                ...
                link.l1 = "Стой! Я приказываю!.. квартирмейстер! очистите трюм...";
		...
		case "free_withoutFee_4":
			...
			if(CheckAttribute(NPChar, "EncType"))
			{
		...
		case "PunishmentAction":
			dialog.text = "Но " + GetAddress_Form(NPChar) + ", вы не можете так поступить! Я же сдался в бою на вашу милость.";
			link.l1 = "После того, как погибли люди, которых вы просто предали... квартирмейстер! приступайте.";
		...
		case "free_tip_off_1":
			...
			link.l1.go = "free_tip_off_0";
			link.l2 = "Не знаю, пригодятся ли мне ваши сведения, но, согласно уговору, вы свободны. квартирмейстер разместит вас с командой, а в ближайшем порту высадит на берег.";
		...
		case "free_by_hoard4":
			...
			link.l1.go = "free_tip_off_0";
			link.l2 = "Хм... интересная история, хотя и верится с трудом. Ну, раз слово дал - идите, скажите квартирмейстеру, пусть выделит вам рундук на баке. В первой же бухте сойдёте на берег.";
		...
		case "free_in_sea":
			...
			link.l1 = "Но вы же именно этого добивались - так что прощайте. квартирмейстер проследит за выполнением моего распоряжения.";
			link.l1.go = "PunishmentAction1";
