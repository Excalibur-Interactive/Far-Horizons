	...
	switch (Dialog.CurrentNode)
	{
		...
		case "ship_tunning_cannon_all_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению калибра и количества орудий на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) +
					" мастер-корабел затребовал: черного дерева - " + NPChar.Tuning.Matherial1 + ", сокровищ - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}				
			else
			{
		...
		case "ship_c_quantity_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: черного дерева - " + NPChar.Tuning.Matherial1 + ", сокровищ - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");				
			}		
			else
			{ 
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_Capacity":
			...
			s1 = s1 + " В оплату за свою работу я возьму: песо слитков - "+ capacityMatherial2 + ", сундуков с золотом - "+ capacityMatherial3 + ",";
		...
		case "ship_tunning_Capacity_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: сандала - " + NPChar.Tuning.Matherial1 + ", песо слитков - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_Capacity_again_2":
			...
			{
				NextDiag.TempNode = "ship_tunning_Capacity_again";
				dialog.Text = "Тебе осталось привезти: сандала - "+ sti(NPChar.Tuning.Matherial1) + ", песо слитков - "+ sti(NPChar.Tuning.Matherial2) + ", сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".";
				link.l1 = "Хорошо.";
				link.l1.go = "Exit";

                AddQuestRecord("ShipTuning", "t1");
				AddQuestUserData("ShipTuning", "sText", "Мне осталось довезти: сандала - "+ sti(NPChar.Tuning.Matherial1) + ", песо слитков - "+ sti(NPChar.Tuning.Matherial2) + ", сундуков - "+ sti(NPChar.Tuning.Matherial3) + ".");
			}
		break;
		...
		case "ship_tunning_SpeedRate_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: шелка - " + NPChar.Tuning.Matherial1 + ", алмазов - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_MaxCrew_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				AddMoneyToCharacter(Pchar, -amount);
			    NPChar.Tuning.Money  = amount;
			    NPChar.Tuning.Matherial1 = MaxCrewMatherial1; // GOOD_CITRUSES
				...
				" мастер-корабел требует: кожи - " + NPChar.Tuning.Matherial1 + ", сапфиров - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_TurnRate_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: полотна - " + NPChar.Tuning.Matherial1 + ", рубинов - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_HP_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: красного дерева - " + NPChar.Tuning.Matherial1 + ", серебряных слитков - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо... Говорит, что в Европу собрался - врёт, наверное, старый черт.");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
		...
		case "ship_tunning_WindAgainst_start":
		    ...
		    if(makeint(Pchar.money) >= amount)
			{
				...
				" мастер-корабел требует: хлопка - " + NPChar.Tuning.Matherial1 + ", изумрудов - "+ NPChar.Tuning.Matherial2+ ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " песо. Совсем старик с ума сбрендил от этих побрякушек.");
			}
			else
			{
				NextDiag.TempNode = "ship_tunning_not_now";
