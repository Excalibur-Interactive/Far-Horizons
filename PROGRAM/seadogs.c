...
void NewGame_continue()
{
	...
	if (startHeroType != 1) LoadMainCharacterInFirstLocationGroup("Ship_deck_Low", "goto", "goto4"); // Rebbebion, вот, где собака зарыта!!!!!! 
	ReloadProgressUpdate();
...
void ProcessControls()
{
	...
	{
		if (CheckAttribute(&InterfaceStack, "SelectMenu_node") && InterfaceStack.SelectMenu_node == "LaunchItems")
		{
			switch(ControlName)
			{
				case "InterfaceF":	Event("Interface_PressControl", "s", "F");	break;
				case "InterfaceC":	Event("Interface_PressControl", "s", "C");	break;
				case "InterfaceX":	Event("Interface_PressControl", "s", "X");	break;
			}
		}
		return;
	}
