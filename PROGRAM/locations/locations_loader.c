...
bool LoadLocation(ref loc)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		{
			LocLoadGrass(loc, "models.always." + sat, numGrass);
			if(numGrass == 1)
			{
				SendMessage(loc, "lsffffffl", MSG_LOCATION_EX_MSG, "SetGrassParams", 1.0, 4.0, 5.0, 10.0, 200.0, 0.0, 1);
			}
			numGrass++;
		}
	}
...
bool LocLoadGrass(aref loc, string sat, int curGrass)	// evganat - трава
{
	string grs = loc.(sat);
	string tex = "";
	sat = sat + ".texture";
	...
	return SendMessage(loc, "lssl", MSG_LOCATION_SET_GRS_PATCH, grs, tex, curGrass);	// evganat - трава
}
...
void ShowAllLocators()
{
	...
	VisibleLocatorsGroup("EliteProtector", 1.0, 15.0, 255, 255, 100, 200); // Rebbebion, новые локаторы для тяжёлой пехоты
	VisibleLocatorsGroup("PatrolElite", 1.0, 15.0, 255, 255, 100, 200); // Rebbebion, новые локаторы для тяжёлой пехоты
...
void HideAllLocators()
{
	...
	HideLocatorsGroup("EliteProtector"); // Rebbebion, новые локаторы для тяжёлой пехоты
	HideLocatorsGroup("PatrolElite"); // Rebbebion, новые локаторы для тяжёлой пехоты
