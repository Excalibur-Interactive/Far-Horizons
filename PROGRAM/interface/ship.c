...
void InitInterface_R(string iniName, ref _chr) // _chr нужно для читового просмотра НПС в море
{
	...
	SetEventHandler("ExitChangeHullMenu", "ExitChangeHullMenu", 0);	// evganat - покраска корпуса
...
void IDoExit(int exitCode)
{
	...
	DelEventHandler("ExitShipChangeMenu", "ExitShipChangeMenu");	// evganat - покраска корпуса
...
void ShowChangeHullMenu()
{
	...
	if(CheckAttribute(rShip, "DisableChangedHull") || CheckAttribute(xi_refCharacter, "PGGAi") || CheckAttribute(xi_refCharacter, "NotShipExit"))
	{
		return;
	}
...
void SetChangeHullInfo()
{
	...
	SetNewPicture("CHANGE_HULL_TYPE", "ships\" + sShipName + "\" + sTypeName + "\" + sTexture + ".tga");
	
	int iMoney = GetCostForChangeHull(rShip, sti(PChar.ChangeHull.Type));
...
string GetShipTexturesForChangeHull()
{
	ref rShip = GetRealShip(sti(xi_refCharacter.Ship.Type));
    	string sShipName = rShip.name;
	string sTex = "bortout" + sShipName;
	
	if(sShipName == "battleship1")
	{
		sTex = "bortoutbatlleship1";
	}
	if(sShipName == "WarTartane1")
	{
		sTex = "bortouttartane1";
	}
	if(sShipName == "FrigateHeavy1")
	{
		sTex = "bortoutfrigate2";
	}
...
void SetNewHullToCharacterShip()
{
	...
	int iMoney = GetCostForChangeHull(refShip, sti(PChar.ChangeHull.Type));
