...
void Ship_CheckSituation()
{
	...
	bool bBalls, bKnippels, bGrapes;
	bBalls = true; bKnippels = true; bGrapes = true;
	// boal -->
	...
	else
	{
	    ...
	    {
	        iNewBallType = Ship_FindOtherBallType(rCharacter, fMinEnemyDistance, bBalls, bGrapes, bKnippels);
	        rCharacter.BOAL_ReadyCharge = "1";
...
int Ship_FindOtherBallType(ref rCharacter, float fMinEnemyDistance, bool bBalls, bool bGrapes, bool bKnippels)
{
	ref	rCannon = GetCannonByType(sti(rCharacter.Ship.Cannons.Type));
	float range = stf(rCannon.FireRange);
...
void CharacterUpdateShipFromBaseShip(int iCharacterIndex)
{
	...
	{ 
		rCharacter.Ship.Cannons.Charge.Type = GOOD_BALLS; 
	}
