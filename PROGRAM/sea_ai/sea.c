...
void DeleteSeaEnvironment()
{
	...
	DeleteClass(&Island);
	DeleteGrass(-1); // трава на остров		evganat - трава
...
void Sea_LoadIsland(string sIslandID)
{
	...
	if (iIslandIndex != -1 && Islands[iIslandIndex].visible == true)
	{
		...
		if( CheckAttribute(&Islands[iIslandIndex],"jungle") )
		{
			...
			CreateGrass("resource\models\islands\"+ Islands[iIslandIndex].id +"\"+ Islands[iIslandIndex].jungle.patch + ".grs", "Grass\"+Islands[iIslandIndex].jungle.texture+".tga", fJungleScale, 20.0, 200.0, 100.0, 1000.0, 0.6, 0); // evganat - трава
		}
