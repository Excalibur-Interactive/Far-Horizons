...
void Fort_Login(int iIslandIndex)
{
	...
	for (int i=0; i<iNumReloads; i++)
	{
		...
		if (CheckAttribute(arLocator,"fort"))
		{
			...
			{
			    SetCharacterGoods(rCharacter, GOOD_BALLS,  300000 + rand(200)); // Rebbebion, нафиг бомбы
			    SetCharacterGoods(rCharacter, GOOD_POWDER, 300000 + rand(200));
			}
			else
			{
			    SetCharacterGoods(rCharacter, GOOD_BALLS,  100 + rand(200)); // Rebbebion, нафиг бомбы
			...
			SetCharacterGoods(rCharacter, GOOD_BRICKS,  130  + rand(900));
...
float Fort_CannonDamage()
{
	...
	if (iNumDamagedCannons >= makeint(iNumAllCannons / (1.05 + 0.19*(10 - MOD_SKILL_ENEMY_RATE)) + 0.1)) // усложним с 2 до 1.5
	{
		Fort_SetAbordageMode(rBallCharacter, rFortCharacter);
  		// 13.05.05 anticheat -->
		SetCharacterGoods(rFortCharacter, GOOD_BALLS,  200 + rand(100));
...
void Fort_CheckAttributes(ref rCharacter)
{
	...
	if (!CheckAttribute(rCharacter, "Ship.Cannons.Charge.Type")) { rCharacter.Ship.Cannons.Charge.Type = GOOD_BALLS; }
	...
	rCharacter.Ship.Cannons.Charge.Type = GOOD_BALLS;  // всегда бомбы   boal / Rebbebion, теперь ядра
}
