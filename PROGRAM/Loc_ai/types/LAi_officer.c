...
void LAi_type_officer_CharacterUpdate(aref chr, float dltTime)
{
	...
	// Rebbebion, возвращение и доработка использования тинктуры офицерами
	if (LAi_IsFightMode(chr) && CheckAttribute(chr, "BerserkerCan") && LAi_GetCharacterRelHP(chr) < 0.8 && LAi_GetCharacterRelEnergy(chr) > 0.3 && GetCharacterItem(chr, "berserker_potion") > 0)
	{
		if(CheckAttribute(chr, "MultiFighter"))
		{
			chr.oldMultiFighter = chr.MultiFighter;
			chr.MultiFighter = chr.MultiFighter * 3.0;
		}
		else chr.MultiFighter = 3.0;
		sTotalTemp = chr.id;
		DoQuestCheckDelay("Officer_BerserkerLock", 12.0);
		RemoveItems(chr, "berserker_potion", 1);
		AddCharacterHealth(chr, -2);
		DeleteAttribute(chr, "BerserkerCan");
		Log_Info("Офицер "+GetFullName(chr)+" использовал тинктуру Бонди!");
	}
