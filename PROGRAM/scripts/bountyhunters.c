...
void SeaHunterCheck()
{
    ...
    for (j=0; j< MAX_NATIONS; j++)
    {
        ...
        if (ChangeCharacterNationReputation(pchar, j, 0) <= -10)// Минус это НЗГ
        {
            ...
            for (i = 1; i <= 7; i++)
            {
                sld = GetCharacter(NPC_GenerateCharacter(sCapId + i, "hunter_" + (rand(15) + 1), "man", "man", 5, j, 20, true, F_MERCENARY));
                SetShipHunter(sld);
                ...
                if (abs(ChangeCharacterNationReputation(pchar, j, 0)) < (i * 15)) break; // добавим капитана только когда положено
            }
	...
	for (j=0; j< MAX_NATIONS; j++)
    {
        ...
        if (GetQuestPastDayParam("Land_HunterTimer" + typeHunter) > (7 + rand(7)))
        {
			...
			if (ChangeCharacterNationReputation(pchar, j, 0) <= -10)// Минус это НЗГ
	        {
	            ...
	            for (i = 1; i <= 8; i++)
	            {
	                sld = GetCharacter(NPC_GenerateCharacter(sCapId + i, "hunter_" + (rand(15) + 1), "man", "man", 5, j, 0, true, F_MERCENARY));
	                SetFantomParamHunter(sld); //крутые парни
