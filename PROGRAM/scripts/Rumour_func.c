...
//============ Осады ===============
	...
	if (CurrentRumour.next != "none" ) // если слух с продолжением
	{
		...
		DeleteRumour(pos);           // сам слух удаляем
...
string SelectRumour() // Получить рандомный слух из очереди
{
    ...
    for(Rumour_Index; Rumour_Index<MAX_RUMOURS; Rumour_Index++)
    {
        ...
        else
        {
            ...
            DeleteRumour(FindRumour(CurrentRumour.id)); // просроченные сразу трем
        }
...
//navy <--
	...
	if (CheckAttribute(rRumour, "sex") && arPrm.sex != rRumour.sex) a = false; //Rosarak
	if (CheckAttribute(rRumour, "group") && !RumourGroupCheck(FindRumour(rRumour.id), arPrm)) a = false; //Rosarak
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
void  DeleteRumour(int del)  //    Удаляет   слух (по номеру) из очереди со сдвигом верхних
{
    object tmp1,tmp2;
    int R_Index;
    ref CurRumour;
	...
	for(int i = 0; i < MAX_RUMOURS; i++)									// to_do: здесь и в RumourGroupCheck лучше через атрибут, а не HasSubStr
	{
		if(!RumourGroupCheck(i,chr))
		{
			...
			if(GetCountSubString(sTemp) > 1)
			{
				...
				else sTemp = StrReplace(sTemp, "," + chr.id, "");
			}
			else sTemp = "";
