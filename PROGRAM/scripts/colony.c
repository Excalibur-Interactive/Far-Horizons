...
void CreateColonyCommanders()
{
	...
	for (i=0; i<MAX_COLONIES; i++)
	{
		...
		UpdateRelations();
  		...
  		characters[iChar].Fort.Cannons.Charge.Type = GOOD_BALLS; // Rebbebion, нафиг бомбы
...
ref SetFantomSlaves(string group, string locator, string enemygroup)
{
    ...
    if (Cr.location.locator != "")
    {
		...
		switch (rand(2))
        {
            case 0: blade = GetGeneratedItem("blade_17"); 			break;
            case 1: blade = GetGeneratedItem("blade_18");       	break;
            case 2: blade = GetGeneratedItem("blade_19");   		break;
        }
...
void HavanaResGatesLock()
{
	int n = FindLocation("Havana_town");
...
void HavanaResGatesUnlock()
{
	int n = FindLocation("Havana_town");
	locations[n].models.night.rinok = "StMartin_rinokN_Capture";
	locations[n].models.night.rinok.tech = "DLightModel";	
	locations[n].models.night.charactersPatch = "StMartin_patch_night_Capture";
}
