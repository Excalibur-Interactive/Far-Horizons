...
// Sith: доработка моря
// Ugeen: туманы от силы ветра (идея Star Noob'а)
//********************************************************************

//базовое значение сетки без учета настроек качества моря
#define LOCATION_GRID_STEP		0.151
#define SEA_GRID_STEP			0.151
...
float SeaDetailsCoeff()
{
	...
	float fSeaDetailsCoeff = 0.1 * (1.0 - stf(InterfaceStates.SeaDetails));
...
bool Whr_CheckLandStorm()
{
	...
	// {
		// if(iIsland != -1) // остров
		// {
			// ref isl = &Islands[iIsland];
			// if(CheckAttribute(isl, "alwaysStorm"))
			// {
				// bool bIslandStorm = isl.alwaysStorm == true;
				// if(bIslandStorm)
				// {
					// return true;
				// }
...
bool Whr_CheckDeck()
{
	...
	if(iLocation != -1)
	{
		...
		{
			bool bOk = 	loc.id.label == "Boarding deck";
			if(bOk)
			{
				return true;
			}
...
int Whr_SetWaterColor(float fMaxSeaHeight)
{
	...
	if(Whr_CheckStorm() || Whr_CheckLandStorm())
	{
		if (GetTime() >=  0.0) 						iWaterColor = argb(  0,  5, 18, 22 );
		if (GetTime() >=  1.0) 						iWaterColor = argb(  0,  5, 18, 22 );
		if (GetTime() >=  2.0) 						iWaterColor = argb(  0,  5, 18, 26 );
		if (GetTime() >=  3.0)						iWaterColor = argb(  0,  5, 23, 27 );
		if (GetTime() >=  4.0)					 	iWaterColor = argb(  0,  5, 25, 29 );
		if (GetTime() >=  5.0) 						iWaterColor = argb(  0,  4, 27, 32 );
		if (GetTime() >=  6.0) 						iWaterColor = argb(  0,  5, 41, 42 );
		if (GetTime() >=  7.0) 						iWaterColor = argb(  0,  5, 44, 48 );
		if (GetTime() >=  8.0 && GetTime() < 19.0) 	iWaterColor = argb(  0,  4, 54, 65 );
		if (GetTime() >= 19.0) 						iWaterColor = argb(  0,  3, 48, 55 );
		if (GetTime() >= 20.0) 						iWaterColor = argb(  0,  3, 35, 42 );
		if (GetTime() >= 21.0) 						iWaterColor = argb(  0,  4, 25, 30 );
		if (GetTime() >= 22.0) 						iWaterColor = argb(  0,  5, 17, 21 );
		if (GetTime() >= 23.0) 						iWaterColor = argb(  0,  5, 16, 20 );		
	}
	else
	{
		// цвет моря в дождь
	...
	{
		// цвет моря на суше с пляжами и в главном меню
		if(Whr_CheckSeashore())
		{
			...
			else
			{
				...
				/* if (GetTime() >=  0.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >=  1.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >=  2.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >=  3.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >=  4.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >=  5.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >=  6.0) iWaterColor = argb(0,5,30,60);
				if (GetTime() >=  7.0) iWaterColor = argb(0,5,30,60);
				if (GetTime() >=  8.0) iWaterColor = argb(0,5,30,60);
				if (GetTime() >=  9.0) iWaterColor = argb(0,5,30,60);
				if (GetTime() >= 10.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 11.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 12.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 13.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 14.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 15.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 16.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 17.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 18.0) iWaterColor = argb(0,0,80,130);
				if (GetTime() >= 19.0) iWaterColor = argb(0,0,55,90);
				if (GetTime() >= 20.0) iWaterColor = argb(0,0,55,90);
				if (GetTime() >= 21.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >= 22.0) iWaterColor = argb(0,5,10,20);
				if (GetTime() >= 23.0) iWaterColor = argb(0,5,10,20); */
			}
		}
	}
...
void Whr_SetSeaFromWind(float fSpeed, float fMaxSeaHeight)
{
	...
	float fFoamK = 1.0 - fSpeed * 0.04;
	if(GetTime() >= 6.0 && GetTime() <= 20.0)
	{
		wRange(&fFoamK, 0.25, 0.5);
	}
	else
	{
		fFoamK = 0.2;
	...
	float fReflection = 1.0 - fSpeed * 0.025;
	float fTransparency = 1.0 - fSpeed * 0.05;
	float fAttenuation = 1.0 - fSpeed * 0.05;
	float fMaxSeaDistance = 3900.0;
	float fBumpScale = -4.0 + (fSpeed);
	wRange(&fBumpScale, 0.05, 0.08);
	if (bMainMenu || fMaxSeaHeight == 3.0 || fMaxSeaHeight == 5.0)
	{
		if (GetTime() >= 6.0 && GetTime() <= 20.0)
		{
			wRange(&fReflection, 0.6, 0.8);	
	...
	else
	{
		if (GetTime() >= 6.0 && GetTime() <= 20.0)
		{
			wRange(&fReflection, 0.5, 0.9);	
			wRange(&fTransparency, 0.1, 0.3);
			wRange(&fAttenuation, 0.3, 0.6);
		}
		else
		{
			wRange(&fReflection, 0.3, 0.4);	
			wRange(&fTransparency, 0.1, 0.3);
			wRange(&fAttenuation, 0.2, 0.4);
		}
	}
	...
	SeaSettings.MaxSeaDistance = fMaxSeaDistance;
...
void Whr_ModifySeaFog( float fWind )
{
	...
	if(Whr_CheckUnderwater()) return;  // под водой это не надо
	
	if (currentHour >= 0.0 && currentHour <= 6.0)
	{
	...
	iDensity 		= 0.00035 + 0.00003 * pow(fWind/7.0, 3) + additionalNightFog;
	sDensity 		= 0.00075 + 0.00003 * pow(fWind/4.5, 3) + additionalNightFog;
	fDensity 		= 0.00035 + 0.00003 * pow(fWind/5.0, 3) + additionalNightFog;
	...
	if(bSeaActive || !bOk1)
	{
		...
		Whr_DebugLog("Fog - Density: " + fDensity + " IDensity: " + iDensity + " SDensity: " + sDensity + " wind:" + fWind);

		weather.Fog.IslandDensity	=	iDensity;
		weather.Fog.SeaDensity		=	sDensity;
		weather.Fog.Density			=	fDensity;
