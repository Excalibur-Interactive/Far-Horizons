	...
	if(CheckAttribute(&WeatherParams,"weather_sky") && !bMainMenu) 	// есть погода текущего дня и не главное меню
	{
		...
		if(dDay > 1)	// генерируем заново, прошло больше 1-го дня
		{
			switch(rand(2))
			{
				...
				case 2: WeatherParams.weather_sky = 3; 	break;	// серьезная облачность				
			}
			isGenerated = true;			
		}			
	}
...
void Whr_ChangeDayNight()
{
	...
	if(loadLocationIndex >= 0) // суша
	{
		...
		LocationSubstituteGeometry(&Locations[loadLocationIndex]);
		if(Whr_IsDay())
		{
		...
		{	
			...
			if(Whr_CheckSeashore() || Whr_CheckSeaTown()) // проверка на тип локации где могут быть установлены корабли
			{	
...
{
	aref 	aCurWeather = GetCurrentWeather();
	float	addRainWindSpeed	= 0.0;

	////// ugeen : TODO здесь нужно что-то посерьезней нежели тупорылый рэндом
	////// ну нельзя такую кривизну писать
	...
	if(CheckAttribute(pchar, "wind.angle"))
    {
		if(rand(1) == 0)
		{	
			Weather.Wind.Angle = stf(pchar.wind.angle) +  frand((PI/6.0)) - (PI / 10.0);  // немного уменьшил дельту угла
	...
	// добавим ветра в дождь
	if(stf(Weather.Wind.Speed) < 10.0)
	{	
		if(wRain == 1)	addRainWindSpeed = frand(3.0) + 3.0;  
		if(wRain == 2)	addRainWindSpeed = frand(4.0) + 4.0;  
	}	
	...
	fWeatherSpeed = stf(Weather.Wind.Speed) + addRainWindSpeed;
}
...
float Whr_SetWindSpeed()
{
	...
	float speed = makefloat(stf(worldMap.WindF) * (WIND_NORMAL_POWER - 2.5));
...
//	fWeatherSpeed = Whr_SetWindSpeed(); //Mett: перерасчет силы ветра
	...
	if (bRain || bWeatherIsStorm)	WhrCreateRainEnvironment();		// итак, если может быть дождь или шторм, создаем окружение
	else							ClearRainEnvironment();			// если не может, удаляем.
